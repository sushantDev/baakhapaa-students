...
class Story with ChangeNotifier {
  ...
  //       } catch (e) {
  //         DebugLogger.info(
  //             '?? notifyListeners() called after disposal (widget navigated away)');
  //       }
  //     } else {
  ...
  //         notifyListeners();
  //       } catch (e) {
  //         DebugLogger.info('?? notifyListeners() called after disposal');
  //       }
  //       throw ('Error');
  //     }
  //   } catch (error) {
  ...
  void _mergeSeasonEpisodeSnapshot(
    ...
    for (final raw in episodes) {
      if (raw is! Map || raw['id'] != episodeId) continue;
      final seasonEp = Map<String, dynamic>.from(raw);
      for (final key in [
        'game_mode',
        'completed_game_modes',
        'completed_modes',
        'game_modes_completed',
        'quiz_completed',
        'crossword_completed',
        'image_puzzle_completed',
      ]) {
        if (seasonEp[key] != null) {
          episodeData[key] = seasonEp[key];
        }
      }
      return;
    }
  ...
  /// Per GET /api/v2/episode/{id}: `watched: true` = quiz completed.
  bool isQuizCompletedFromEpisode(Map<String, dynamic> episodeData) {
    return _isTruthy(episodeData['watched']);
  }

  static bool _isModeCompletedFlag(
    Map<String, dynamic> episodeData,
    GameMode gameMode,
  ) {
    final apiName = gameMode.toApiString();
    final keys = <String>[
      '${apiName}_completed',
      '${apiName}_watched',
      if (gameMode == GameMode.quiz) 'quiz_completed',
      if (gameMode == GameMode.crossword) 'crossword_completed',
      if (gameMode == GameMode.imagePuzzle) 'image_puzzle_completed',
    ];
    return keys.any((key) => _isTruthy(episodeData[key]));
  }
  ...
  List<GameMode> completedGameModesFromEpisode(Map<String, dynamic> episodeData) {
    ...
    if (isQuizCompletedFromEpisode(episodeData)) {
      completed.add(GameMode.quiz);
    }

    final fromList = episodeData['completed_game_modes'] ??
        episodeData['completed_modes'] ??
        episodeData['game_modes_completed'];
    if (fromList is List) {
      for (final entry in fromList) {
        completed.add(GameMode.fromApiString(entry?.toString()));
      }
    }

    for (final mode in [GameMode.crossword, GameMode.imagePuzzle]) {
      if (_isModeCompletedFlag(episodeData, mode)) {
        completed.add(mode);
      }
    }

    final rawMode = episodeData['game_mode']?.toString().trim();
    if (rawMode != null && rawMode.isNotEmpty) {
      final mode = GameMode.fromApiString(rawMode);
  ...
  bool isGameModeCompletedLocally(
    Map<String, dynamic> episodeData,
    GameMode gameMode,
  ) {
    return completedGameModesFromEpisode(episodeData).contains(gameMode);
  }
  ...
  Future<bool> hasCompletedEpisodeQuiz(int episodeId) async {
    ...
    return isQuizCompletedFromEpisode(_episode);
  }

  bool episodeHasQuizQuestions(Map<String, dynamic> episodeData) {
    final questions = episodeData['questions'];
    return questions is List && questions.isNotEmpty;
  }

  Future<List<GameMode>> availableGameModesForEpisode(int episodeId) async {
    try {
      await fetchEpisode(episodeId);
    } catch (error) {
      DebugLogger.error(
          'availableGameModesForEpisode: fetch failed for $episodeId: $error');
      if (_episode['id'] != episodeId) {
        return [GameMode.quiz, GameMode.crossword, GameMode.imagePuzzle];
      }
    }

    final episodeData = Map<String, dynamic>.from(_episode);
    _mergeSeasonEpisodeSnapshot(episodeId, episodeData);

    final completed = completedGameModesFromEpisode(episodeData);
    final available = <GameMode>[];

    for (final mode in GameMode.values) {
      if (completed.contains(mode)) continue;
      if (mode == GameMode.quiz && !episodeHasQuizQuestions(episodeData)) {
        continue;
      }
      available.add(mode);
    }
  ...
  void _markGameModeCompletedOnEpisodeMap(
    Map<String, dynamic> episodeMap,
    GameMode completedMode,
  ) {
    if (completedMode == GameMode.quiz) {
      episodeMap['watched'] = true;
    }
    episodeMap['game_mode'] = completedMode.toApiString();

    final existing = episodeMap['completed_game_modes'];
    final modes = existing is List
        ? existing.map((e) => e.toString()).toList()
        : <String>[];
    final apiName = completedMode.toApiString();
    if (!modes.contains(apiName)) {
      modes.add(apiName);
    }
    episodeMap['completed_game_modes'] = modes;
  }

  /// Updates challenge completion locally for immediate UI feedback.
  void updateEpisodeWatchedStatusLocally(int episodeId) {
    try {
      final completedMode = _selectedGameMode;

      if (_selectedSeason['episodes'] != null) {
        final episodes = _selectedSeason['episodes'] as List<dynamic>;
        for (int i = 0; i < episodes.length; i++) {
          if (episodes[i] is Map && episodes[i]['id'] == episodeId) {
            final ep = Map<String, dynamic>.from(episodes[i] as Map);
            _markGameModeCompletedOnEpisodeMap(ep, completedMode);
            episodes[i] = ep;
            DebugLogger.success(
              'Story Provider: Marked $episodeId ${completedMode.toApiString()} completed locally',
            );
            break;
          }
        }
      ...
      if (_episode['id'] == episodeId) {
        _markGameModeCompletedOnEpisodeMap(_episode, completedMode);
        DebugLogger.success(
          'Story Provider: Marked current episode $episodeId ${completedMode.toApiString()} completed locally',
        );
      }
  ...
  Map<String, dynamic> _normalizeSeasonData(Map<String, dynamic> seasonData) {
    ...
    if (seasonData['creator'] != null && seasonData['creator'] is Map) {
      ...
      if (normalized['user_image'] == null ||
        ...
        DebugLogger.warning(
            '?? Season data missing creator object, cannot extract creator image');
      }
    }
  ...
  Future<void> fetchContinueWatching() async {
    ...
    try {
      DebugLogger.api(
        '?? STARTING fetchContinueWatching - authToken length: ${authToken.length}',
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true &&
          ...
          DebugLogger.api(
            '?? Loaded ${_continueWatchingItems.length} continue watching items',
          );

          // Debug: Log first item
          if (_continueWatchingItems.isNotEmpty) {
            DebugLogger.api(
              '?? First item: ${_continueWatchingItems.first['season']['title']}, '
          ...
          DebugLogger.api(
            '?? Continue Watching API Error: ${responseData['message'] ?? 'Unknown error'}',
          );
        ...
        DebugLogger.api(
          '?? Continue Watching API Error: ${response.statusCode} - ${response.body}',
        );
        _continueWatchingItems = [];
        _isLoadingContinueWatching = false;
        notifyListeners();
