...
4. [API Implementation Guide](#4-api-implementation-guide)
5. [Content Linking System](#5-content-linking-system)
...
| ------------------- | ---------------------------- | -------------------------------------------------------- |
| **During Creation** | Creating new shorts/episodes | `POST /api/shorts` or `POST /api/episodes`               |
| **After Creation**  | Adding to existing content   | `POST /api/link-to-short` or `POST /api/link-to-episode` |
...
### 2.2 Product Link Types

| Link Type | Purpose                    | Commission | Who Can Use              |
| --------- | -------------------------- | ---------- | ------------------------ |
| featured  | Regular product promotion  | ❌ No      | All creators             |
| Affiliate | Commission-based promotion | ✅ Yes     | Approved affiliates only |
| related   | Content recommendations    | ❌ No      | All creators             |

---

## 3. Complete User Flow

### 3.1 Prerequisite: Become an Affiliate

Before linking affiliate products, creators must join the affiliate program:
...
└─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘
       ...
       │                  │                  │                  │
       ▼                  ▼                  ▼                  ▼
POST /program/join    Pending...      Auto-approve      Commission!
```

**API Call:**

```bash
POST /api/program/join
...
### 3.2 Flow A: Link Products DURING Content Creation
...
**Endpoint:** `POST /api/shorts` (or `/api/episodes`)
**Request Body:**
...
```json
{
  ...
  // ═══════════════════════════════════════════
  "season_id": 10, // Link to a series
  "related_shorts_ids": [23, 45, 67], // Related videos
...
#### What Happens in the Backend (Shorts)

```php
// In ShortsController@store() method
DB::transaction(function () use ($request) {
    // 1. Calculate coins and check balance
    $calculatedCoins = $request->get('coins') * $request->get('coins_users');
    if ($info->available_coins < $calculatedCoins) return false;

    // 2. Create the shorts
    $shorts = Shorts::create($request->myData());

    // 3. Deduct coins and log transaction
    $info->update(['available_coins' => $info->available_coins - $calculatedCoins]);
    CoinLog::create([...]);

    // 4. Handle Affiliate Products
    if ($request->filled('affiliate_product_ids')) {
        // Validate user is approved affiliate
        $isApproved = AffiliateProgramRequest::where('user_id', auth()->id())
            ->where('status', 'approved')->exists();

        if (!$isApproved) throw new Exception('Must be approved affiliate');

        foreach ($request->affiliate_product_ids as $productId) {
            $product = Product::find($productId);
            if (!$product->is_affiliable || $product->affiliate_commission <= 0) {
                throw new Exception("Product {$productId} not available for affiliate");
            }

            // Create/Update AffiliateRequest for tracking
            AffiliateRequest::firstOrCreate(
                ['user_id' => auth()->id(), 'product_id' => $productId, 'shorts_id' => $shorts->id],
                ['status' => 'approved', 'affiliate_commission' => $product->affiliate_commission]
            );

            // Link product with 'Affiliate' type
            $shorts->linkProduct($productId, 'Affiliate');
        }
    }

    // 5. Handle featured Products & related Content
    if ($request->filled('product_ids')) $shorts->linkProducts($request->product_ids, 'featured');
    if ($request->filled('season_id')) $shorts->linkSeason($request->season_id, 'featured');
    if ($request->filled('related_shorts_ids')) {
        foreach ($request->related_shorts_ids as $id) $shorts->linkShorts($id, 'related');
    }

    // 6. Handle Challenge Logic
    // If show_shorts is false, it's a challenge submission.
    // We link it to the user's pending challenge in challenge_user table.
    if ($request->input('show_shorts') === 'false') {
        $pivotEntry = DB::table('challenge_user')
            ->where('user_id', auth()->id())
            ->whereNull('challenge_shorts_id')->first();
        if ($pivotEntry) {
            DB::table('challenge_user')->where('user_id', auth()->id())
                ->where('challenge_id', $pivotEntry->challenge_id)
                ->update(['challenge_shorts_id' => $shorts->id]);
        }
    }
});
```

#### What Happens in the Backend (Episodes)

```php
// In EpisodeController@store() method
DB::transaction(function () use ($request) {
    // 1. Logic for coins deduction identical to Shorts
    // 2. Create the episode
    $episode = Episode::create($request->myData());

    // 3. Attach Featured Products (Many-to-Many)
    $productIds = $request->getProductIds();
    if (!empty($productIds)) {
        $episode->products()->attach($productIds);
    }

    // 4. Handle Affiliate Products (ContentLink System)
    if ($request->filled('affiliate_product_ids')) {
        // Validation logic same as Shorts...
        foreach ($request->affiliate_product_ids as $productId) {
            // Create AffiliateRequest tracking...
            // Link product with 'Affiliate' type using ContentLink
            $episode->linkProduct($productId, 'Affiliate');
        }
    }
});
```
...
### 3.3 Flow B: Link Products AFTER Content Creation
...
│     ↓                                                                │
│  5. Call Link API                                                   │
│     POST /api/link-to-short                                         │
│     OR                                                               │
│     POST /api/link-to-episode                                       │
│     ↓                                                                │
...
**Endpoint:** `POST /api/link-to-short`
**Request Body:**
...
// 6. Link product to shorts
ContentLink::create([
    'content_type' => 'Shorts',
    'content_id' => $shorts->id,
...
### 3.4 Comparison: During vs After Creation

| Feature               | During Creation    | After Creation            |
| --------------------- | ------------------ | ------------------------- |
| **Endpoint**          | `POST /api/shorts` | `POST /api/link-to-short` |
...
| ------ | ------------------------- | ------------------------------- |
| POST   | `/api/program/join`       | Join affiliate program          |
| GET    | `/api/available-products` | Get affiliable products         |
| GET    | `/api/creator-requests`   | Get affiliation status          |
| POST   | `/api/shorts`             | Create shorts with products     |
| POST   | `/api/link-to-short`      | Add product to existing short   |
| POST   | `/api/link-to-episode`    | Add product to existing episode |
...
content_links
├── id
├── content_type       → 'Shorts' or 'Episode'
├── content_id         → ID of the shorts/episode
├── linkable_type      → 'Product', 'Season', or 'Shorts'
├── linkable_id        → ID of the linked item
├── link_type          → 'Affiliate', 'Featured', 'Related'
└── timestamps
```

### 5.2 Link Types Explained
...
- ✅ Any creator can use
- ✅ No approval needed
- ✅ Good for personal product showcases
...
- ❌ Not for products
- ✅ Links to other shorts/episodes
- ✅ Helps content discovery
- ✅ No approval needed

### 5.3 How Linked Content Appears

#### A. Shorts (v2) Response

When fetching shorts via `GET /api/shorts_v2`, the response includes complex linking:
...
When fetching episodes via `GET /api/episodes/{seasonId}` or `GET /api/episode/{id}`:
...
### 8.4 Automated Content Workflow
...
### 9.1 Content Creation Guidelines
